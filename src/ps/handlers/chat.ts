...
export function aliasedArgsToName (aliasArgs: string[]): { command: PSCommand, context: PSCommandContext } {
...
export default async function chatHandler (message: Message) {
	...
	try {
		...
		context.args = [...command, ...args];
		context.run = function (altArgText: string, ctx: Record<string, any> = {}) {
			const altArgs = altArgText.split(' ');
			const { command, context } = aliasedArgsToName(altArgs);
