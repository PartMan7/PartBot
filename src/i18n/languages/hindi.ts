...
export default {
	...
	GAME: {
		...
		AZUL: {
			...
			CHOOSE_WALL_COLUMN: 'Apne wall tile ke liye column chunein.',
			PLACE_TILES: 'Apne tiles pattern line ya penalties par rakhein.',
			SELECT_ROW: 'Rakhne ke liye row chunein.',
			CHOOSE_WALL_COLUMN_UI: 'Wall column chunein:',
			UNRECOGNIZED_ACTION: 'Unrecognized action {{action}}',
			NO_TILES_IN_CENTER: 'Center mein {{tile}} tiles nahi hain.',
			INVALID_FACTORY: 'Invalid factory.',
			NO_TILES_IN_FACTORY: 'Us factory mein {{tile}} tiles nahi hain.',
			NO_TILES_TO_PLACE: 'Aapke paas rakhne ke liye tiles nahi hain.',
			CHOOSE_PATTERN_OR_FLOOR: 'Pattern line (0-4) ya penalties chunein.',
			CANNOT_PLACE_ON_ROW: 'Woh pattern line ye tiles nahi le sakti.',
			INVALID_TILE: '{{tile}} valid tile type nahi hai.',
			NO_WALL_PENDING: 'Koi wall placement pending nahi hai.',
			NOT_YOUR_WALL: 'Yeh aapka wall placement nahi hai.',
			ILLEGAL_WALL_COLUMN: 'Yeh column is tile ke liye legal nahi hai.',
			EXTRA_TO_PENALTIES: 'Extra tiles penalties mein jayenge.',
			LOG: {
				PLACED: '{{count}}x rakha',
				ON_PENALTIES: 'penalties par',
				ON_ROW: 'row {{row}} par',
				OVERFLOW: ' ({{count}} penalties mein)',
				TILED: 'tiled',
				FOR_POINTS: '{{points}} ke liye.',
				SKIPPED: 'skip kiya.',
				POINT: 'point',
				POINTS: 'points',
			},
